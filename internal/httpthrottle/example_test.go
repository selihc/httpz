...
func ExampleHTTPRateLimiter() {
	...
	quota := httpthrottle.RateQuota{MaxRate: httpthrottle.PerMin(20), MaxBurst: 5}

	rateLimiter, err := httpthrottle.NewGCRARateLimiter(store, quota)
	if err != nil {
		log.Fatal(err)
	}

	httpRateLimiter := httpthrottle.HTTPRateLimiter{
		RateLimiter: rateLimiter,
		VaryBy:      &httpthrottle.VaryBy{Path: true},
	}
...
func ExampleGCRARateLimiter() {
	...
	quota := httpthrottle.RateQuota{MaxRate: httpthrottle.PerHour(1), MaxBurst: 5}
