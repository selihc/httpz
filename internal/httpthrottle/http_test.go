...
func (sl *stubLimiter) RateLimit(key string, quantity int) (bool, httpthrottle.RateLimitResult, error) {
	switch key {
	case "limit":
		result := httpthrottle.RateLimitResult{
			Limit:      -1,
			Remaining:  -1,
			ResetAfter: -1,
...
func TestHTTPRateLimiter(t *testing.T) {
	limiter := httpthrottle.HTTPRateLimiter{
