...
import (
	...
	"selihc.com/glaive/httpz/httpthrottle"
	"selihc.com/glaive/httpz/httpthrottle/memstore"
)

const deniedStatus = http.StatusTooManyRequests

type testStore struct {
	store httpthrottle.GCRAStore
...
func TestRateLimit(t *testing.T) {
	limit := 5
	rq := httpthrottle.RateQuota{MaxRate: httpthrottle.PerSec(1), MaxBurst: limit - 1}
	start := time.Unix(0, 0)
	cases := []struct {
		now               time.Time
	...
	rl, err := httpthrottle.NewGCRARateLimiter(&st, rq)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRateLimitCustomPeriod(t *testing.T) {
	...
	rq := httpthrottle.RateQuota{httpthrottle.PerDuration(3, period), 0}
...
func TestRateLimitUpdateFailures(t *testing.T) {
	rq := httpthrottle.RateQuota{MaxRate: httpthrottle.PerSec(1), MaxBurst: 1}
	mst, err := memstore.New(0)
	if err != nil {
		t.Fatal(err)
	}
...
func BenchmarkRateLimit(b *testing.B) {
	limit := 5
	rq := httpthrottle.RateQuota{MaxRate: httpthrottle.PerSec(1000), MaxBurst: limit - 1}
	mst, err := memstore.New(0)
	if err != nil {
		b.Fatal(err)
	}
